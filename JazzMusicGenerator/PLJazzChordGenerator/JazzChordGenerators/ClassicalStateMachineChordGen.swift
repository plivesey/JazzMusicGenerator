...
class ClassicalStateMachineChordGen: ChordGenProtocol {
  ...
  func firstChord(chord: ChordData) -> (state: ChordNumber, transposition: Int) {
    // Represts the chord number and the transposition
    var possibleAnswers: [(ChordNumber, Int)] = []
    if chord.type == ChordType.Major7 {
      // Could be a I or a IV
      possibleAnswers = [
        (.I, chord.baseNote),
        (.IV, (chord.baseNote - 5 + 12) % 12)]
    } else if chord.type == ChordType.Minor7 {
      possibleAnswers = [
        (.ii, (chord.baseNote - 2 + 12) % 12),
        (.iii, (chord.baseNote - 4 + 12) % 12),
        (.vi, (chord.baseNote - 9 + 12) % 12)]
    } else if chord.type == ChordType.Dom7 {
      possibleAnswers = [(.V, (chord.baseNote - 7 + 12) % 12)]
    } else if chord.type == ChordType.DimPartial {
      possibleAnswers = [(.vii, (chord.baseNote + 1) % 12)]
    }
